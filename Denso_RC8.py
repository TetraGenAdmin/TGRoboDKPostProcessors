...
exec("from v" + version_str + ".Denso_RC8 import RobotPost as BasePost")

class RobotPost(BasePost):

    # ------------------------ Customize your RoboDK post processor using the following variables ------------------------

    # Maximum number of lines per program
    MAX_LINES_X_PROG = -1

    # Export targets with respect to the robot when a turntable is used
    # This is turned on automatically if we have a turntable
    TurnTableExport_RobotBase = False
...
if __name__== "__main__":
    exec("from v" + version_str + ".Denso_RC8 import test_post")
    test_post()
