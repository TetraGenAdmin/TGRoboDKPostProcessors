...
#   Universal Robots RobotiQ robot controllers
#
...
exec("from v" + version_str + ".Universal_Robots_RobotiQ import RobotPost as BasePost")
...
class RobotPost(BasePost):
  ...
  def rq_wait_for_vacuum_object_suction_complete(gripper_socket="1"):
    ...
    while (not rq_is_vacuum_obj_in_suction(gripper_socket) and not rq_is_vacuum_obj_detected(gripper_socket) and remaining_retries > 0):
      sleep(0.01)
...
if __name__== "__main__":
    exec("from v" + version_str + ".Universal_Robots_RobotiQ import test_post")
    test_post()
